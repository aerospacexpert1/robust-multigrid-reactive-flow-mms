...
cd "$ROOT"
[[ -x build/opposedflow_finalvol2 ]] || ./build.sh
rm -rf smoke_output
...
./build/opposedflow_finalvol2 \
  -Nx 108 -Ny 36 \
  -end 0.02 -dt 1e-7 -dtMax 1e-4 \
  -vFuel 0.05 -vOx -0.05 \
  -threads "$OMP_NUM_THREADS" -caseId RMT_FINALVOL2_SMOKE \
...
grep -q 'RMT_FINALVOL2_DIAGNOSTICS' smoke_output/smoke.log
...
print('RMT_FINALVOL2_SMOKE PASS')
