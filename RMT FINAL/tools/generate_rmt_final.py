#!/usr/bin/env python3
"""Generate RMT FINAL from the frozen V95 SG-RBGS production source.
...
if len(sys.argv) != 3:
    raise SystemExit("usage: generate_rmt_final.py FROZEN_V95.c OUTPUT.c")
...
s = s.replace(marker, '#include "rmt_final2d_impl.h"\n\n' + marker, 1)
...
new_pressure = r'''static PressureStats solve_pressure_poisson(const Grid *g, const Phys *ph, Fields *f,
    ...
    RMTFinalWorkspace *ws=rmt_final_workspace(g);
    ...
    if (fabs(c->mgOmega-1.0) > 1.0e-14)
        die("RMT FINAL requires the full Martynenko correction (rmtOmega=1)");
    ...
    for (int cyc=0; cyc<c->poissonIters; ++cyc) {
        ...
        RMTFinalDiag d = {0,0,0,0,0};
        rmt_final_build_correction(g, defect, corr, &cfg, &d);
        ...
        if (!isfinite(newRes))
            die("RMT FINAL produced a non-finite pressure residual");

        const double ratio = newRes/MAX(oldRes,1.0e-300);
        g_rmtFinalLastCycleRatio = ratio;
        g_rmtFinalMaxCycleRatio = MAX(g_rmtFinalMaxCycleRatio,ratio);
        if (newRes > oldRes) ++g_rmtFinalNonmonotoneCycles;
    ...
    if (stats.absResidual > c->pressureAbsTol && stats.relResidual > c->pressureRelTol) {
        ...
        die("RMT FINAL pressure solve hit max cycles without meeting the frozen V95 tolerance");
    }
...
s = s.replace("SG_RBGS", "RMT_FINAL")
s = s.replace("SG-RBGS", "RMT FINAL")
...
s = s.replace(
    'printf("RMT FINAL: omega=%g, maxSweeps=%d, threads=%d\\n", c.mgOmega, c.poissonIters, c.threads);',
    'printf("RMT FINAL: smoothSweeps=%d, maxCycles=%d, threads=%d\\n", c.mgPostSmooth, c.poissonIters, c.threads);'
)
...
insert = (
    ...
    "        ? (double)g_rmtFinalPointUpdatesTotal/((double)g->Nx*(double)g->Ny) : 0.0;\n"
...
diag = (
    '    printf("RMT_FINAL_DIAGNOSTICS levelsX=%d levelsY=%d direct_solves=%lld direct_unknowns=%lld '
    'nonmonotone_cycles=%lld last_cycle_ratio=%.17g max_cycle_ratio=%.17g point_updates=%lld\\n",\n'
    '           g_rmtFinalLastLevelX, g_rmtFinalLastLevelY,\n'
    '           g_rmtFinalDirectSolvesTotal, g_rmtFinalDirectUnknownsTotal,\n'
    '           g_rmtFinalNonmonotoneCycles, g_rmtFinalLastCycleRatio,\n'
    '           g_rmtFinalMaxCycleRatio, g_rmtFinalPointUpdatesTotal);\n'
)
if anchor not in s:
    raise SystemExit("generation refused: final diagnostics anchor missing")
s = s.replace(anchor, diag + anchor, 1)

banner = "// RMT_FINAL_GENERATED: frozen V95 physics/timestep; pressure linear solver only replaced by RMT.\n"
