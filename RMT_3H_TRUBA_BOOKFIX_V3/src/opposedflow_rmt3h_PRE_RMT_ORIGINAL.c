...
static double arrhenius_k(const Phys *ph, double T) {
    ...
    if (logk > 700.0) logk = 700.0; if (logk < -700.0) return 0.0; return exp(logk);
}
...
int main(int argc, char **argv) {
    ...
    printf("V94-RMT-pressureRMTStableScalars: serial 2D 9-grid RMT on pressure with stable V91 scalar/enthalpy blocks\n");
    ...
    double time=c.startTime, dt=c.dt0, nextWrite=c.writeInterval; int step=0; write_vti(&g,&ph,&f,time); write_centerline_csv(&g,&f,time);
