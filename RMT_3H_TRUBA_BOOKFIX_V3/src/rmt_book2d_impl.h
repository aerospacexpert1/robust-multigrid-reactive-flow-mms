...
/*
 * Finest-grid pressure-correction operator.
 *
 * Production pressure is cell-centred.  The x patches impose fixed pressure,
 * therefore the homogeneous correction has Dirichlet c=0 at x=0,Lx.
 * The y patches impose zeroGradient pressure, therefore dc/dn=0 at y=0,Ly.
 *
 * Eliminating the ghost cells gives, at the first/last x cell, a 3/h^2
 * diagonal contribution and a 1/h^2 interior-neighbour contribution.  At a
 * zero-Neumann y boundary the contribution is 1/h^2 on the diagonal and
 * 1/h^2 to the interior neighbour.  This routine is the algebraic reference
 * that every RMT level must reduce to when sx=sy=1.
 */
...
/*
 * Cell-centred finite-volume boundary elimination on a shifted RMT line.
 *
 * H is the spacing between two unknowns on the current shifted grid.  dist is
 * the physical distance from the current unknown to the missing boundary.
 * The boundary control-volume width is V = dist + H/2.  Integrating -u_xx
 * over that CV gives
 *
 *   Dirichlet: [ (uP-uB)/dist - (uE-uP)/H ] / V
 *   Neumann 0: [              - (uE-uP)/H ] / V
 *
 * (and the mirrored formula on the opposite boundary).  For H=h and
 * dist=h/2 these reduce exactly to the production cell-centred ghost-cell
 * stencil: 3/h^2 at a Dirichlet boundary and 1/h^2 at a zero-Neumann
 * boundary.  Thus the finest RMT level solves exactly the same linear
 * operator as the production pressure equation; no comparison tolerance or
 * governing equation is changed by this BOOKFIX.
 */
    ...
    *ap += aNbr + aBoundary;
    ...
    if (!hm && hp)
        ...
        *am = aNbr;
        ...
        /* This hierarchy is normally stopped before a shifted line contains
         * only one point.  Retain a positive local operator as a defensive
         * fallback rather than silently using a vertex-centred formula. */
        *ap += aBoundary;
    }
}
        ...
        for (int gid=0; gid<ng; ++gid) {
            ...
            for (int i=ox; i<=g->Nx; i+=sx)
                for (int j=oy; j<=g->Ny; j+=sy) {
                    c[IDX(i,j,g->Ny)] =
                        rmt_book_update_point(g,c,rhs,i,j,sx,sy);
                    ++updates;
                }
        }
    }
...
static void rmt_book_build_correction(const Grid *g, const double *b, double *c,
    ...
    /* Martynenko-style sawtooth: no presmoothing.  The same defect equation
     * is represented on each level; the correction computed on coarse
     * shifted grids is retained as the initial guess on successively finer
     * levels. */
    for (int lc=lm; lc>=0; --lc) {
        ...
        int sw = (lc == lm) ? MAX(1,cfg->coarseSweeps)
