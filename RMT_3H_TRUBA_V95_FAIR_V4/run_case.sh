...
EXE="$ROOT/build/opposedflow_rmt3h_v95_fair"
...
mkdir -p "$run_dir"
if [[ -f "$run_dir/RUN_COMPLETE" ]]; then echo "SKIP completed: $case_id"; exit 0; fi
rm -rf "$out_dir"; rm -f "$run_dir/RUN_FAILED" "$run_dir/RUN_COMPLETE"; mkdir -p "$out_dir"
...
solver=RMT_3H_BOOK_V95_FAIR_V4
rmt_levels=$RMT_LEVELS
rmt_post_sweeps=$RMT_POST
rmt_coarsest_sweeps=$RMT_COARSE_SWEEPS
rmt_initial_omega=$RMT_OMEGA
rmt_omega_policy=full_correction_first_then_half_on_residual_increase_no_hidden_fallback
...
cmd=("$EXE"
  ...
  -rmtLevels "$RMT_LEVELS" -rmtPost "$RMT_POST" -rmtCoarseSweeps "$RMT_COARSE_SWEEPS" -rmtOmega "$RMT_OMEGA"
...
if [[ -n "${SLURM_JOB_ID:-}" ]] && command -v srun >/dev/null 2>&1; then
  srun --nodes=1 --ntasks=1 --cpus-per-task="$threads" --exclusive --cpu-bind=cores "${cmd[@]}" > "$run_dir/run.log" 2>&1
  rc=$?
else
