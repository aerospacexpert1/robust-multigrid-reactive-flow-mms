/*
  ...
  Three-level classical geometric multigrid V-cycle pressure solver for the
  ...
  Each V-cycle uses red-black Gauss-Seidel smoothing, cell-volume restriction,
  ...
  Compile:
    gcc -O3 -march=native -std=c11 -Wall -Wextra -Wpedantic -fopenmp \
        opposedflow_mg2v.c -lm -o opposedflow_mg2v

  Example:
    OMP_NUM_THREADS=4 ./opposedflow_mg2v \
...
static void parse_args(int argc, char **argv, Grid *g, Phys *ph, Controls *c) {
    ...
    if ((g->Nx % 4) != 0 || (g->Ny % 4) != 0) die("MG2V requires Nx and Ny divisible by 4 for the h/2h/4h hierarchy");
...
static MG2VWorkspace g_mg2v = {0};

static size_t grid_storage_size(const Grid *g) {
    return (size_t)(g->Nx + 2) * (size_t)(g->Ny + 2);
}

static void mg2v_workspace_release(void) {
    free(g_mg2v.fineResidual);
    free(g_mg2v.level1Rhs);
    free(g_mg2v.level1Correction);
    free(g_mg2v.level1Residual);
    free(g_mg2v.level2Rhs);
    free(g_mg2v.level2Correction);
    memset(&g_mg2v, 0, sizeof(g_mg2v));
}
...
static void mg2v_workspace_prepare(const Grid *fine) {
    if (g_mg2v.fineResidual && g_mg2v.fineNx == fine->Nx && g_mg2v.fineNy == fine->Ny) return;

    mg2v_workspace_release();
    g_mg2v.fineNx = fine->Nx;
    g_mg2v.fineNy = fine->Ny;
    g_mg2v.level1 = mg_make_coarse_grid(fine);
    g_mg2v.level2 = mg_make_coarse_grid(&g_mg2v.level1);

    const size_t n0 = grid_storage_size(fine);
    const size_t n1 = grid_storage_size(&g_mg2v.level1);
    const size_t n2 = grid_storage_size(&g_mg2v.level2);

    g_mg2v.fineResidual    = (double*)calloc(n0, sizeof(double));
    g_mg2v.level1Rhs       = (double*)calloc(n1, sizeof(double));
    g_mg2v.level1Correction= (double*)calloc(n1, sizeof(double));
    g_mg2v.level1Residual  = (double*)calloc(n1, sizeof(double));
    g_mg2v.level2Rhs       = (double*)calloc(n2, sizeof(double));
    g_mg2v.level2Correction= (double*)calloc(n2, sizeof(double));

    if (!g_mg2v.fineResidual || !g_mg2v.level1Rhs || !g_mg2v.level1Correction ||
        !g_mg2v.level1Residual || !g_mg2v.level2Rhs || !g_mg2v.level2Correction)
        die("allocate MG2V workspace");
}
...
static void mg_level1_vcycle(const Controls *c, MGWorkStats *work) {
    Grid *g1 = &g_mg2v.level1;
    Grid *g2 = &g_mg2v.level2;
    const size_t n2 = grid_storage_size(g2);

    mg_rbgs_correction(g1, g_mg2v.level1Correction, g_mg2v.level1Rhs,
                       c->mgPreSmooth, c->mgOmega);
    work->level1PreSweeps += c->mgPreSmooth;

    mg_residual_correction(g1, g_mg2v.level1Correction,
                           g_mg2v.level1Rhs, g_mg2v.level1Residual);
    mg_restrict_2x2(g1, g_mg2v.level1Residual, g2, g_mg2v.level2Rhs);
    ++work->restrictions;

    memset(g_mg2v.level2Correction, 0, n2*sizeof(double));
    mg_rbgs_correction(g2, g_mg2v.level2Correction, g_mg2v.level2Rhs,
                       c->mgCoarseSweeps, c->mgOmega);
    work->coarseSweeps += c->mgCoarseSweeps;

    mg_prolong_bilinear_add(g2, g_mg2v.level2Correction,
                            g1, g_mg2v.level1Correction);
    ++work->prolongations;
...
static PressureStats solve_pressure_poisson(const Grid *g, const Phys *ph, Fields *f,
    ...
    mg2v_workspace_prepare(g);
    cycle_history_prepare(history, c->poissonIters + 1);
    history->count = 0;

    apply_bc_velocity_pressure(g, ph, f);
    mg_residual_pressure_fine(g, f, f->rhs, g_mg2v.fineResidual);

    stats.rhsNorm = max_abs_interior(g, f->rhs);
    stats.initialResidual = mg_max_abs(g, g_mg2v.fineResidual);
    ...
    for (int cyc=0; cyc<c->poissonIters; ++cyc) {
        ...
        mg_residual_pressure_fine(g, f, f->rhs, g_mg2v.fineResidual);
        mg_restrict_2x2(g, g_mg2v.fineResidual,
                        &g_mg2v.level1, g_mg2v.level1Rhs);
        ++work->restrictions;

        memset(g_mg2v.level1Correction, 0, n1*sizeof(double));
        mg_level1_vcycle(c, work);

        /* Correct the fine pressure and post-smooth. */
        mg_prolong_bilinear_add(&g_mg2v.level1, g_mg2v.level1Correction,
                                g, f->p);
        ++work->prolongations;
        apply_bc_velocity_pressure(g, ph, f);
        ...
        mg_residual_pressure_fine(g, f, f->rhs, g_mg2v.fineResidual);
        stats.cycles = cyc + 1;
        stats.absResidual = mg_max_abs(g, g_mg2v.fineResidual);
        stats.relResidual = stats.absResidual / MAX(stats.rhsNorm, 1.0e-30);
...
static void write_case_summary_csv(const Grid *g, const Phys *ph, const Controls *c,
    ...
    fprintf(fp,
        "%s,MG2V_3LEVEL,%d,%d,%d,%lld,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.17g,%.17g,%d,"
...
int main(int argc, char **argv) {
    ...
    printf("MG2V-3LEVEL reacting-flow solver: frozen V95 physics with a classical pressure V-cycle\n");
    printf("Case ID: %s\n", c.caseId);
