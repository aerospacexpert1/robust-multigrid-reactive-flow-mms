/*
  ...
  Three-level classical geometric multigrid W-cycle pressure solver for the
  ...
  replaced by a conventional three-level geometric multigrid W-cycle (gamma=2):

      level 0: h     (fine grid)
      level 1: 2h
      level 2: 4h    (coarsest grid)

  Each W-cycle uses red-black Gauss-Seidel smoothing, cell-volume restriction,
  cell-centred bilinear prolongation and a coarse-grid iterative solve.  The
  default is 3 pre-smoothing sweeps, 3 post-smoothing sweeps and 40 coarsest-grid
  sweeps.  All four thesis meshes are divisible by four, so the hierarchy is exact.
  ...
  Performance rules:
    ...
    - total W-cycle count and multigrid work counts are reported;
  ...
  Compile:
    gcc -O3 -march=native -std=c11 -Wall -Wextra -Wpedantic -fopenmp \
        opposedflow_mg2w.c -lm -o opposedflow_mg2w

  Example:
    OMP_NUM_THREADS=4 ./opposedflow_mg2w \
...
static void parse_args(int argc, char **argv, Grid *g, Phys *ph, Controls *c) {
    ...
    if ((g->Nx % 4) != 0 || (g->Ny % 4) != 0) die("MG2W requires Nx and Ny divisible by 4 for the h/2h/4h hierarchy");
...
static MG2WWorkspace g_mg2w = {0};

static size_t grid_storage_size(const Grid *g) {
    return (size_t)(g->Nx + 2) * (size_t)(g->Ny + 2);
}

static void mg2w_workspace_release(void) {
    free(g_mg2w.fineResidual);
    free(g_mg2w.level1Rhs);
    free(g_mg2w.level1Correction);
    free(g_mg2w.level1Residual);
    free(g_mg2w.level2Rhs);
    free(g_mg2w.level2Correction);
    memset(&g_mg2w, 0, sizeof(g_mg2w));
}
...
static Grid mg_make_coarse_grid(const Grid *fine) {
    ...
    coarse.Nx = fine->Nx / 2;
    coarse.Ny = fine->Ny / 2;
    coarse.dx = fine->dx * 2.0;
    coarse.dy = fine->dy * 2.0;
    return coarse;
}

static void mg2w_workspace_prepare(const Grid *fine) {
    if (g_mg2w.fineResidual && g_mg2w.fineNx == fine->Nx && g_mg2w.fineNy == fine->Ny) return;

    mg2w_workspace_release();
    g_mg2w.fineNx = fine->Nx;
    g_mg2w.fineNy = fine->Ny;
    g_mg2w.level1 = mg_make_coarse_grid(fine);
    g_mg2w.level2 = mg_make_coarse_grid(&g_mg2w.level1);

    const size_t n0 = grid_storage_size(fine);
    const size_t n1 = grid_storage_size(&g_mg2w.level1);
    const size_t n2 = grid_storage_size(&g_mg2w.level2);

    g_mg2w.fineResidual    = (double*)calloc(n0, sizeof(double));
    g_mg2w.level1Rhs       = (double*)calloc(n1, sizeof(double));
    g_mg2w.level1Correction= (double*)calloc(n1, sizeof(double));
    g_mg2w.level1Residual  = (double*)calloc(n1, sizeof(double));
    g_mg2w.level2Rhs       = (double*)calloc(n2, sizeof(double));
    g_mg2w.level2Correction= (double*)calloc(n2, sizeof(double));

    if (!g_mg2w.fineResidual || !g_mg2w.level1Rhs || !g_mg2w.level1Correction ||
        !g_mg2w.level1Residual || !g_mg2w.level2Rhs || !g_mg2w.level2Correction)
        die("allocate MG2W workspace");
}
...
/* Cell-volume restriction.  A coarse cell contains four fine cells. */
static void mg_restrict_2x2(const Grid *fine, const double *rf,
    ...
    for (int ic=1; ic<=coarse->Nx; ++ic) for (int jc=1; jc<=coarse->Ny; ++jc) {
        int i0 = 2*ic - 1;
        int j0 = 2*jc - 1;
        bc[IDX(ic,jc,coarse->Ny)] = 0.25*(
              rf[IDX(i0,  j0,  fine->Ny)] + rf[IDX(i0+1,j0,  fine->Ny)]
            + rf[IDX(i0,  j0+1,fine->Ny)] + rf[IDX(i0+1,j0+1,fine->Ny)]);
    }
}

/* Cell-centred bilinear prolongation.  Fine children are located one quarter
   of a coarse-cell spacing from their parent centre, giving 3/4--1/4 weights. */
static void mg_prolong_bilinear_add(const Grid *coarse, double *ec,
                                   const Grid *fine, double *xf) {
    mg_apply_correction_bc(coarse, ec);

    #pragma omp parallel for collapse(2) schedule(static)
    for (int ic=1; ic<=coarse->Nx; ++ic) for (int jc=1; jc<=coarse->Ny; ++jc) {
        const int il = 2*ic - 1, ir = 2*ic;
        const int jb = 2*jc - 1, jt = 2*jc;
        ...
        xf[IDX(il,jb,fine->Ny)] += 0.5625*c + 0.1875*cw + 0.1875*cs + 0.0625*csw;
        xf[IDX(ir,jb,fine->Ny)] += 0.5625*c + 0.1875*ce + 0.1875*cs + 0.0625*cse;
        xf[IDX(il,jt,fine->Ny)] += 0.5625*c + 0.1875*cw + 0.1875*cn + 0.0625*cnw;
        xf[IDX(ir,jt,fine->Ny)] += 0.5625*c + 0.1875*ce + 0.1875*cn + 0.0625*cne;
    }
}
...
/* One level-1 coarse-correction pass through level 2.
   A classical W-cycle invokes this pass twice (gamma=2) before returning to L0. */
static void mg_level1_cycle(const Controls *c, MGWorkStats *work) {
    Grid *g1 = &g_mg2w.level1;
    Grid *g2 = &g_mg2w.level2;
    const size_t n2 = grid_storage_size(g2);

    mg_rbgs_correction(g1, g_mg2w.level1Correction, g_mg2w.level1Rhs,
                       c->mgPreSmooth, c->mgOmega);
    work->level1PreSweeps += c->mgPreSmooth;

    mg_residual_correction(g1, g_mg2w.level1Correction,
                           g_mg2w.level1Rhs, g_mg2w.level1Residual);
    mg_restrict_2x2(g1, g_mg2w.level1Residual, g2, g_mg2w.level2Rhs);
    ++work->restrictions;

    memset(g_mg2w.level2Correction, 0, n2*sizeof(double));
    mg_rbgs_correction(g2, g_mg2w.level2Correction, g_mg2w.level2Rhs,
                       c->mgCoarseSweeps, c->mgOmega);
    work->coarseSweeps += c->mgCoarseSweeps;

    mg_prolong_bilinear_add(g2, g_mg2w.level2Correction,
                            g1, g_mg2w.level1Correction);
    ++work->prolongations;
...
static PressureStats solve_pressure_poisson(const Grid *g, const Phys *ph, Fields *f,
    ...
    mg2w_workspace_prepare(g);
    cycle_history_prepare(history, c->poissonIters + 1);
    history->count = 0;

    apply_bc_velocity_pressure(g, ph, f);
    mg_residual_pressure_fine(g, f, f->rhs, g_mg2w.fineResidual);

    stats.rhsNorm = max_abs_interior(g, f->rhs);
    stats.initialResidual = mg_max_abs(g, g_mg2w.fineResidual);
    ...
    for (int cyc=0; cyc<c->poissonIters; ++cyc) {
        ...
        mg_residual_pressure_fine(g, f, f->rhs, g_mg2w.fineResidual);
        mg_restrict_2x2(g, g_mg2w.fineResidual,
                        &g_mg2w.level1, g_mg2w.level1Rhs);
        ++work->restrictions;

        memset(g_mg2w.level1Correction, 0, n1*sizeof(double));

        /* Classical W-cycle: gamma=2 recursive visits to the L1 branch.
           The second visit starts from the corrected L1 approximation left
           by the first visit, recomputes its residual, and revisits L2. */
        for (int gamma=0; gamma<MG_W_GAMMA; ++gamma) {
            mg_level1_cycle(c, work);
        }

        /* Correct the fine pressure and post-smooth. */
        mg_prolong_bilinear_add(&g_mg2w.level1, g_mg2w.level1Correction,
                                g, f->p);
        ++work->prolongations;
        apply_bc_velocity_pressure(g, ph, f);
        ...
        mg_residual_pressure_fine(g, f, f->rhs, g_mg2w.fineResidual);
        stats.cycles = cyc + 1;
        stats.absResidual = mg_max_abs(g, g_mg2w.fineResidual);
        stats.relResidual = stats.absResidual / MAX(stats.rhsNorm, 1.0e-30);
...
static void write_case_summary_csv(const Grid *g, const Phys *ph, const Controls *c,
    ...
    fprintf(fp,
        ...
        "pressure_solves,total_wcycles,avg_wcycles_per_pressure_solve,max_wcycles_per_pressure_solve,"
    ...
    const double equivalentFineSweeps =
      ...
      + 0.25*(double)(work->level1PreSweeps + work->level1PostSweeps)
      + 0.0625*(double)work->coarseSweeps;
    const double equivalentFineSweepsPerSolve = pressureSolves > 0
      ? equivalentFineSweeps/(double)pressureSolves : 0.0;

    fprintf(fp,
        "%s,MG2W_3LEVEL,%d,%d,%d,%lld,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.17g,%.17g,%d,"
        ...
        g->Nx, g->Ny, g->Nx/2, g->Ny/2, g->Nx/4, g->Ny/4,
...
int main(int argc, char **argv) {
    ...
    printf("MG2W-3LEVEL reacting-flow solver: frozen V95 physics with a classical pressure W-cycle (gamma=2)\n");
    printf("Case ID: %s\n", c.caseId);
    printf("Pressure hierarchy: L0=%dx%d, L1=%dx%d, L2=%dx%d (factor-two full coarsening)\n",
           g.Nx,g.Ny,g.Nx/2,g.Ny/2,g.Nx/4,g.Ny/4);
