/*
  ...
  Three-level factor-three classical geometric multigrid V-cycle pressure solver for the
  structured-grid opposed-flow reacting-flow code used in the thesis campaign.

  The reacting-flow formulation and physical model are inherited from the frozen
  V95 OpenFOAM-matched in-house solver.  Only the pressure Poisson algorithm is
  replaced by a conventional three-level geometric multigrid V-cycle:

      level 0: h     (fine grid)
      level 1: 3h
      level 2: 9h    (coarsest grid)

  Each V-cycle uses red-black Gauss-Seidel smoothing, conservative 3x3
  cell-volume restriction, cell-centred bilinear prolongation for a 3:1 grid
  ratio, and a coarse-grid iterative solve.  The default is 3 pre-smoothing
  sweeps, 3 post-smoothing sweeps and 40 coarsest-grid sweeps.  All four thesis
  meshes are divisible by nine, so the hierarchy is exact.

  OpenMP is used for the structured-grid loops.  The same executable is therefore
  used for the 1, 2, 4 and 16 thread runs in the 96-case TRUBA campaign.

  Performance rules:
    - solver compute time excludes VTK/CSV/performance-file output;
    - pressure time is reported separately;
    - total V-cycle count and multigrid work counts are reported;
  ...
  Compile:
    gcc -O3 -march=native -std=c11 -Wall -Wextra -Wpedantic -fopenmp \
        opposedflow_mg3v.c -lm -o opposedflow_mg3v

  Example:
    OMP_NUM_THREADS=4 ./opposedflow_mg3v \
...
typedef struct {
    ...
    int poissonIters;                  /* maximum pressure cycles */
    int scalarIters;                   /* scalar cycles */
    int scalarSweepsPerCycle;

    /* Classical three-level V-cycle controls. */
    int mgPreSmooth;
    int mgPostSmooth;
    int mgCoarseSweeps;
    double mgOmega;
...
typedef struct {
    long long restrictions;
    long long prolongations;
    long long finePreSweeps;
    long long finePostSweeps;
    long long level1PreSweeps;
    long long level1PostSweeps;
    long long coarseSweeps;
} MGWorkStats;
...
static void parse_args(int argc, char **argv, Grid *g, Phys *ph, Controls *c) {
    for (int i=1; i<argc; ++i) {
        ...
        else if (!strcmp(argv[i], "-mgOmega") && i+1<argc) c->mgOmega = atof(argv[++i]);
    ...
    if ((g->Nx % 9) != 0 || (g->Ny % 9) != 0) die("MG3V requires Nx and Ny divisible by 9 for the h/3h/9h hierarchy");
    ...
    if (c->mgPreSmooth < 0 || c->mgPostSmooth < 0 || c->mgCoarseSweeps < 1) die("invalid multigrid sweep controls");
    if (c->mgOmega <= 0.0 || c->mgOmega > 2.0) die("mgOmega must be in (0,2]");
...
/* ------------------------------------------------------------------------- */
/* Classical three-level geometric multigrid pressure solver                 */
/* ------------------------------------------------------------------------- */
...
typedef struct {
    ...
    double *fineResidual;
    double *level1Rhs;
    double *level1Correction;
    double *level1Residual;
    double *level2Rhs;
    double *level2Correction;
} MG3VWorkspace;

static MG3VWorkspace g_mg3v = {0};

static size_t grid_storage_size(const Grid *g) {
    return (size_t)(g->Nx + 2) * (size_t)(g->Ny + 2);
}

static void mg3v_workspace_release(void) {
    free(g_mg3v.fineResidual);
    free(g_mg3v.level1Rhs);
    free(g_mg3v.level1Correction);
    free(g_mg3v.level1Residual);
    free(g_mg3v.level2Rhs);
    free(g_mg3v.level2Correction);
    memset(&g_mg3v, 0, sizeof(g_mg3v));
}

static Grid mg_make_coarse_grid(const Grid *fine) {
    Grid coarse = *fine;
    coarse.Nx = fine->Nx / 3;
    coarse.Ny = fine->Ny / 3;
    coarse.dx = fine->dx * 3.0;
    coarse.dy = fine->dy * 3.0;
    return coarse;
}

static void mg3v_workspace_prepare(const Grid *fine) {
    if (g_mg3v.fineResidual && g_mg3v.fineNx == fine->Nx && g_mg3v.fineNy == fine->Ny) return;

    mg3v_workspace_release();
    g_mg3v.fineNx = fine->Nx;
    g_mg3v.fineNy = fine->Ny;
    g_mg3v.level1 = mg_make_coarse_grid(fine);
    g_mg3v.level2 = mg_make_coarse_grid(&g_mg3v.level1);

    const size_t n0 = grid_storage_size(fine);
    const size_t n1 = grid_storage_size(&g_mg3v.level1);
    const size_t n2 = grid_storage_size(&g_mg3v.level2);

    g_mg3v.fineResidual    = (double*)calloc(n0, sizeof(double));
    g_mg3v.level1Rhs       = (double*)calloc(n1, sizeof(double));
    g_mg3v.level1Correction= (double*)calloc(n1, sizeof(double));
    g_mg3v.level1Residual  = (double*)calloc(n1, sizeof(double));
    g_mg3v.level2Rhs       = (double*)calloc(n2, sizeof(double));
    g_mg3v.level2Correction= (double*)calloc(n2, sizeof(double));

    if (!g_mg3v.fineResidual || !g_mg3v.level1Rhs || !g_mg3v.level1Correction ||
        !g_mg3v.level1Residual || !g_mg3v.level2Rhs || !g_mg3v.level2Correction)
        die("allocate MG3V workspace");
}

/* Homogeneous correction boundary conditions corresponding to the pressure
   boundary conditions of the fine-grid problem: zero correction on the two
   pressure outlets and zero normal-gradient correction on top/bottom. */
static void mg_apply_correction_bc(const Grid *g, double *x) {
    const int nx = g->Nx, ny = g->Ny;

    for (int j=1; j<=ny; ++j) {
        x[IDX(0,    j,ny)] = -x[IDX(1, j,ny)];
        x[IDX(nx+1, j,ny)] = -x[IDX(nx,j,ny)];
    }
    for (int i=1; i<=nx; ++i) {
...
static void mg_rbgs_correction(const Grid *g, double *x, const double *b,
    ...
    for (int sweep=0; sweep<sweeps; ++sweep) {
        for (int color=0; color<2; ++color) {
            mg_apply_correction_bc(g, x);
            #pragma omp parallel for collapse(2) schedule(static)
            for (int i=1; i<=nx; ++i) for (int j=1; j<=ny; ++j) {
                if (((i+j)&1) != color) continue;
                double gs = (b[IDX(i,j,ny)]
                           + ax*(x[IDX(i+1,j,ny)] + x[IDX(i-1,j,ny)])
                           + ay*(x[IDX(i,j+1,ny)] + x[IDX(i,j-1,ny)])) / ap;
                x[IDX(i,j,ny)] = (1.0-omega)*x[IDX(i,j,ny)] + omega*gs;
            }
        }
    }
    mg_apply_correction_bc(g, x);
...
static void mg_rbgs_pressure_fine(const Grid *g, const Phys *ph, Fields *f,
...
static void mg_residual_correction(const Grid *g, const double *x,
    ...
    #pragma omp parallel for collapse(2) schedule(static)
    for (int i=1; i<=nx; ++i) for (int j=1; j<=ny; ++j) {
        double Ax = ap*x[IDX(i,j,ny)]
                  - ax*(x[IDX(i+1,j,ny)] + x[IDX(i-1,j,ny)])
                  - ay*(x[IDX(i,j+1,ny)] + x[IDX(i,j-1,ny)]);
        r[IDX(i,j,ny)] = b[IDX(i,j,ny)] - Ax;
    }
}

static void mg_residual_pressure_fine(const Grid *g, const Fields *f,
...
/* Conservative cell-volume restriction for factor-three full coarsening.
   Each coarse cell contains a 3x3 block of fine cells. */
static void mg_restrict_3x3(const Grid *fine, const double *rf,
                            const Grid *coarse, double *bc) {
    memset(bc, 0, grid_storage_size(coarse)*sizeof(double));
    #pragma omp parallel for collapse(2) schedule(static)
    for (int ic=1; ic<=coarse->Nx; ++ic) for (int jc=1; jc<=coarse->Ny; ++jc) {
        const int i0 = 3*ic - 2;
        const int j0 = 3*jc - 2;
        double sum = 0.0;
        for (int di=0; di<3; ++di)
            for (int dj=0; dj<3; ++dj)
                sum += rf[IDX(i0+di,j0+dj,fine->Ny)];
        bc[IDX(ic,jc,coarse->Ny)] = sum / 9.0;
    }
}

/* Cell-centred bilinear prolongation for a 3:1 grid ratio.
   The middle fine-cell centre coincides with the parent coarse-cell centre.
   The two off-centre children lie one third of a coarse spacing toward the
   neighbouring coarse centre, giving 2/3--1/3 one-dimensional weights. */
static void mg_prolong_bilinear_3to1_add(const Grid *coarse, double *ec,
                                         const Grid *fine, double *xf) {
    mg_apply_correction_bc(coarse, ec);

    #pragma omp parallel for collapse(2) schedule(static)
    for (int ic=1; ic<=coarse->Nx; ++ic) for (int jc=1; jc<=coarse->Ny; ++jc) {
        const double c   = ec[IDX(ic,  jc,  coarse->Ny)];
        const double cw  = ec[IDX(ic-1,jc,  coarse->Ny)];
        const double ce  = ec[IDX(ic+1,jc,  coarse->Ny)];
        const double cs  = ec[IDX(ic,  jc-1,coarse->Ny)];
        const double cn  = ec[IDX(ic,  jc+1,coarse->Ny)];
        const double csw = ec[IDX(ic-1,jc-1,coarse->Ny)];
        const double cse = ec[IDX(ic+1,jc-1,coarse->Ny)];
        const double cnw = ec[IDX(ic-1,jc+1,coarse->Ny)];
        const double cne = ec[IDX(ic+1,jc+1,coarse->Ny)];

        const int iL = 3*ic - 2, iC = 3*ic - 1, iR = 3*ic;
        const int jB = 3*jc - 2, jC = 3*jc - 1, jT = 3*jc;

        /* centre row */
        xf[IDX(iL,jC,fine->Ny)] += (2.0/3.0)*c + (1.0/3.0)*cw;
        xf[IDX(iC,jC,fine->Ny)] += c;
        xf[IDX(iR,jC,fine->Ny)] += (2.0/3.0)*c + (1.0/3.0)*ce;

        /* bottom row: tensor product of x interpolation with 2/3 current + 1/3 south */
        xf[IDX(iL,jB,fine->Ny)] += (4.0/9.0)*c + (2.0/9.0)*cw + (2.0/9.0)*cs + (1.0/9.0)*csw;
        xf[IDX(iC,jB,fine->Ny)] += (2.0/3.0)*c + (1.0/3.0)*cs;
        xf[IDX(iR,jB,fine->Ny)] += (4.0/9.0)*c + (2.0/9.0)*ce + (2.0/9.0)*cs + (1.0/9.0)*cse;

        /* top row: tensor product of x interpolation with 2/3 current + 1/3 north */
        xf[IDX(iL,jT,fine->Ny)] += (4.0/9.0)*c + (2.0/9.0)*cw + (2.0/9.0)*cn + (1.0/9.0)*cnw;
        xf[IDX(iC,jT,fine->Ny)] += (2.0/3.0)*c + (1.0/3.0)*cn;
        xf[IDX(iR,jT,fine->Ny)] += (4.0/9.0)*c + (2.0/9.0)*ce + (2.0/9.0)*cn + (1.0/9.0)*cne;
    }
}

static double mg_max_abs(const Grid *g, const double *a) {
    double value = 0.0;
...
/* Solve the level-1 correction equation with one V-cycle through level 2. */
static void mg_level1_vcycle(const Controls *c, MGWorkStats *work) {
    Grid *g1 = &g_mg3v.level1;
    Grid *g2 = &g_mg3v.level2;
    const size_t n2 = grid_storage_size(g2);

    mg_rbgs_correction(g1, g_mg3v.level1Correction, g_mg3v.level1Rhs,
                       c->mgPreSmooth, c->mgOmega);
    work->level1PreSweeps += c->mgPreSmooth;

    mg_residual_correction(g1, g_mg3v.level1Correction,
                           g_mg3v.level1Rhs, g_mg3v.level1Residual);
    mg_restrict_3x3(g1, g_mg3v.level1Residual, g2, g_mg3v.level2Rhs);
    ++work->restrictions;

    memset(g_mg3v.level2Correction, 0, n2*sizeof(double));
    mg_rbgs_correction(g2, g_mg3v.level2Correction, g_mg3v.level2Rhs,
                       c->mgCoarseSweeps, c->mgOmega);
    work->coarseSweeps += c->mgCoarseSweeps;

    mg_prolong_bilinear_3to1_add(g2, g_mg3v.level2Correction,
                            g1, g_mg3v.level1Correction);
    ++work->prolongations;

    mg_rbgs_correction(g1, g_mg3v.level1Correction, g_mg3v.level1Rhs,
                       c->mgPostSmooth, c->mgOmega);
    work->level1PostSweeps += c->mgPostSmooth;
}
...
static PressureStats solve_pressure_poisson(const Grid *g, const Phys *ph, Fields *f,
    ...
    mg3v_workspace_prepare(g);
    cycle_history_prepare(history, c->poissonIters + 1);
    history->count = 0;

    apply_bc_velocity_pressure(g, ph, f);
    mg_residual_pressure_fine(g, f, f->rhs, g_mg3v.fineResidual);

    stats.rhsNorm = max_abs_interior(g, f->rhs);
    stats.initialResidual = mg_max_abs(g, g_mg3v.fineResidual);
    ...
    for (int cyc=0; cyc<c->poissonIters; ++cyc) {
        ...
        mg_rbgs_pressure_fine(g, ph, f, f->rhs, c->mgPreSmooth, c->mgOmega);
        work->finePreSweeps += c->mgPreSmooth;

        /* Fine residual -> level 1. */
        mg_residual_pressure_fine(g, f, f->rhs, g_mg3v.fineResidual);
        mg_restrict_3x3(g, g_mg3v.fineResidual,
                        &g_mg3v.level1, g_mg3v.level1Rhs);
        ++work->restrictions;

        memset(g_mg3v.level1Correction, 0, n1*sizeof(double));
        mg_level1_vcycle(c, work);

        /* Correct the fine pressure and post-smooth. */
        mg_prolong_bilinear_3to1_add(&g_mg3v.level1, g_mg3v.level1Correction,
                                g, f->p);
        ++work->prolongations;
        apply_bc_velocity_pressure(g, ph, f);

        mg_rbgs_pressure_fine(g, ph, f, f->rhs, c->mgPostSmooth, c->mgOmega);
        work->finePostSweeps += c->mgPostSmooth;

        mg_residual_pressure_fine(g, f, f->rhs, g_mg3v.fineResidual);
        stats.cycles = cyc + 1;
        stats.absResidual = mg_max_abs(g, g_mg3v.fineResidual);
        stats.relResidual = stats.absResidual / MAX(stats.rhsNorm, 1.0e-30);
...
static void solve_scalar_helmholtz(const Grid *g, const Phys *ph, const Fields *f, double *phi, const double *rhs,
    ...
    /*
       ...
       attributable to the multigrid pressure treatment.
    */
...
static void write_pressure_step_history(const Controls *c, const PressureStepHistory *h) {
    ...
    fprintf(fp, "step,time_s,dt_s,cycles,initial_residual,final_residual,final_relative_residual,convergence_factor\n");
...
static void write_case_summary_csv(const Grid *g, const Phys *ph, const Controls *c,
                                   ...
                                   const MGWorkStats *work, const MassStats *mass) {
    char path[PATH_MAX];
    ...
    fprintf(fp,
        "case_id,solver,threads,Nx,Ny,cells,level0_Nx,level0_Ny,level1_Nx,level1_Ny,level2_Nx,level2_Ny,"
        "mg_pre_sweeps,mg_post_sweeps,mg_coarse_sweeps,mg_omega,final_time_s,time_steps,"
        ...
        "pressure_solves,total_vcycles,avg_vcycles_per_pressure_solve,max_vcycles_per_pressure_solve,"
        ...
        "final_pressure_convergence_factor,restrictions,prolongations,fine_pre_sweeps,fine_post_sweeps,"
        "level1_pre_sweeps,level1_post_sweeps,coarse_sweeps,equivalent_fine_grid_smoothing_sweeps,equivalent_fine_grid_smoothing_sweeps_per_pressure_solve,"
    ...
    const double equivalentFineSweeps =
        (double)(work->finePreSweeps + work->finePostSweeps)
      + (1.0/9.0)*(double)(work->level1PreSweeps + work->level1PostSweeps)
      + (1.0/81.0)*(double)work->coarseSweeps;
    const double equivalentFineSweepsPerSolve = pressureSolves > 0
      ? equivalentFineSweeps/(double)pressureSolves : 0.0;

    fprintf(fp,
        "%s,MG3V_3LEVEL,%d,%d,%d,%lld,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.17g,%.17g,%d,"
        "%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,"
        "%.17g,%.17g,%.17g,%.17g,%.17g,%lld,%lld,%.17g,%d,%.17g,%d,%.17g,%.17g,%.17g,%.17g,%.17g,"
        "%lld,%lld,%lld,%lld,%lld,%lld,%lld,%.17g,%.17g,"
        "%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g,%.17g\n",
        c->caseId, c->threads, g->Nx, g->Ny, (long long)g->Nx*(long long)g->Ny,
        g->Nx, g->Ny, g->Nx/3, g->Ny/3, g->Nx/9, g->Ny/9,
        c->mgPreSmooth, c->mgPostSmooth, c->mgCoarseSweeps, c->mgOmega,
        finalTime, steps,
        ...
        work->restrictions, work->prolongations, work->finePreSweeps, work->finePostSweeps,
        work->level1PreSweeps, work->level1PostSweeps, work->coarseSweeps,
        equivalentFineSweeps, equivalentFineSweepsPerSolve,
        mass->maxAbs, mass->l2, mass->integral,
...
int main(int argc, char **argv) {
    ...
    printf("MG3V-3LEVEL reacting-flow solver: frozen V95 physics with a classical pressure V-cycle\n");
    printf("Case ID: %s\n", c.caseId);
    printf("Pressure hierarchy: L0=%dx%d, L1=%dx%d, L2=%dx%d (factor-three full coarsening)\n",
           g.Nx,g.Ny,g.Nx/3,g.Ny/3,g.Nx/9,g.Ny/9);
    printf("MG cycle: pre=%d RBGS, post=%d RBGS, coarse=%d RBGS, omega=%g, threads=%d\n",
           c.mgPreSmooth,c.mgPostSmooth,c.mgCoarseSweeps,c.mgOmega,c.threads);
    ...
    printf("Pressure maxCycles=%d relTol=%.3e absTol=%.3e\n",
    ...
    MGWorkStats totalMGWork = {0,0,0,0,0,0,0};
    ...
    while (time < c.endTime - 1e-15) {
        ...
        {
            MGWorkStats stepWork = {0,0,0,0,0,0,0};
            struct timespec p_t0, p_t1;
            timer_now(&p_t0);
            pStats = solve_pressure_poisson(&g,&ph,&f,&c,&cycleHistory,&stepWork);
            timer_now(&p_t1);
            ...
            totalMGWork.restrictions += stepWork.restrictions;
            totalMGWork.prolongations += stepWork.prolongations;
            totalMGWork.finePreSweeps += stepWork.finePreSweeps;
            totalMGWork.finePostSweeps += stepWork.finePostSweeps;
            totalMGWork.level1PreSweeps += stepWork.level1PreSweeps;
            totalMGWork.level1PostSweeps += stepWork.level1PostSweeps;
            totalMGWork.coarseSweeps += stepWork.coarseSweeps;
        }
        ...
        if (progressNow) {
            ...
            printf("step=%7d t=%10.6f dt=%9.3e Co=%8.2e Di=%8.2e pCyc=%3d pRel=%9.2e rhoMG=%7.4f massMax=%9.2e massL2=%9.2e Tmax=%9.2f YPmax=%8.5f YOmax=%8.5f YFmax=%8.5f\n",
    ...
    if (c.writeOutput) {
        ...
        write_case_summary_csv(&g,&ph,&c,&f,time,step,
                               ...
                               meanConvergenceFactor,&pStats,&totalMGWork,&mStats);
        timer_now(&out_t1);
        outputElapsed += elapsed_seconds(&out_t0,&out_t1);
    }

    printf("Done. Final time=%g steps=%d\n",time,step);
    printf("Final pressure: cycles=%d initialResidual=%.6e absResidual=%.6e relResidual=%.6e rhsNorm=%.6e rhoMG=%.6f\n",
           pStats.cycles,pStats.initialResidual,pStats.absResidual,pStats.relResidual,pStats.rhsNorm,pStats.convergenceFactor);
    printf("Pressure totals: solves=%lld cycles=%lld avgCycles=%.6f maxCycles=%d pressureTime=%.6f s meanRhoMG=%.6f\n",
    ...
    printf("MG work: restrictions=%lld prolongations=%lld finePre=%lld finePost=%lld level1Pre=%lld level1Post=%lld coarseSweeps=%lld\n",
           totalMGWork.restrictions,totalMGWork.prolongations,totalMGWork.finePreSweeps,totalMGWork.finePostSweeps,
           totalMGWork.level1PreSweeps,totalMGWork.level1PostSweeps,totalMGWork.coarseSweeps);
