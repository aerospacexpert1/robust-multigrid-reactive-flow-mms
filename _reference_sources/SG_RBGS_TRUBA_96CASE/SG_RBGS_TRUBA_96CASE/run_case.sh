...
EXE="$ROOT/build/opposedflow_sg_rbgs"
...
# Matched physical/numerical settings used by all in-house pressure-solver campaigns.
...
# Single-grid RBGS pressure controls.
RBGS_OMEGA=${RBGS_OMEGA:-1.0}
P_SWEEPS=${P_SWEEPS:-500000}
...
solver=SG_RBGS
level0=${Nx}x${Ny}
rbgs_omega=$RBGS_OMEGA
rbgs_max_sweeps=$P_SWEEPS
...
srun --nodes=1 --ntasks=1 --cpus-per-task="$threads" --exclusive --cpu-bind=cores \
    ...
    -rbgsOmega "$RBGS_OMEGA" \
    -pCycles "$P_SWEEPS" -pRelTol "$P_REL_TOL" -pAbsTol "$P_ABS_TOL" \
